...
// GET /api/proposals/groups/[groupId] - Get proposals by group
  ...
  const handler = createAPIHandler(
    async (request: NextRequest) => {
      try {
        ...
        const proposals = await proxyToBackend(`/proposals/groups/${groupId}?${queryParams.toString()}`, {
          method: 'GET',
        }, token);

        return createSuccessResponse(proposals, 200, 'Proposals retrieved successfully');
      } catch (error: any) {
        console.error('Get proposals by group error:', error);
        ...
        return createErrorResponse('Failed to retrieve proposals', 500, 'INTERNAL_ERROR');
      }
    },
    {
      requireAuth: true,
