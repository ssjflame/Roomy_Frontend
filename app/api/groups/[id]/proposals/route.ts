...
// GET /api/groups/[id]/proposals - Get group proposals
  ...
  const handler = createAPIHandler(
    async (request: NextRequest) => {
      try {
        ...
        const proposals = await proxyToBackend(`/groups/${id}/proposals?${queryParams.toString()}`, {
          method: 'GET',
        }, token);

        return createSuccessResponse(proposals, 200, 'Group proposals retrieved successfully');
      } catch (error: any) {
        ...
        return createErrorResponse('Failed to retrieve group proposals', 500, 'INTERNAL_ERROR');
      }
    },
    {
      requireAuth: true,
